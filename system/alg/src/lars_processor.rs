use core::{ModelInfo, Note, Tone, generate_keys};
...
impl LarsProcessor {
    pub fn new(info: &ModelInfo, models_dir: &std::path::Path) -> Self {
        let model_path = models_dir.join(&info.filename);
        let td = TrainingData::load(&model_path)
